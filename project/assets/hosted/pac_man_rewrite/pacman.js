class player { //defining the player
    ...
    update() { //update loop
        ...
        if (this.pos.x > textMap[0].length) {
            this.pos.x = -1
        } //screenwrap
        if (this.pos.x < -1) {
            this.pos.x = textMap[0].length
        }

        if (this.pos.y > textMap.length) {
            this.pos.y = -1
        }
        if (this.pos.y < -1) {
            this.pos.y = textMap.length
        }
    ...
    show() { //display event
        ...
        if (useSprites) {
            if (this.dead) {
                ...
                if (ticks % 5 == 0) {
                    this.animDir *= this.anim == 0 || this.anim == 2 ? -1 : 1
                }
        ...
        if (this.dead) {
            anim = min(179, max((this.deadTicks - 60) * 2, 1))
        } //if im dead make it keep going up instead
